...
void solve() {
    int n, k; cin >> n >> k;
    vi pre(n,0); cin >> pre[0];
    FOR(i,1,n){int x; cin >> x; pre[i] = pre[i-1] + x;}
    debug(pre);
    int ans = 0;
    FOR(i,0,n){
        int target = 0;
        if(i == 0) target = pre[0] + k-pre[0];
        else target = k - (pre[i] - pre[i-1]) + pre[i];
        int v = bs_k_lon_nhat__NHO_hon_target(pre, target, i, n-1ll);
        // cout << i << ", "  << target << ", "  << v << "\n";
        ans = max(ans, v - i + 1ll);
    }
    cout << ans << "\n";
